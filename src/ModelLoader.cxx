  ...
  std::string const& p_name)
{
  ...
  {    
    ...
    loadedModel->AddMeshMaterialLink(meshIdx, materialMapping[scene->mMeshes[meshIdx]->mMaterialIndex]);
  }
...
{
  auto LoadColorType = [p_aiMaterial, this](const char* p_Key, unsigned int p_type, unsigned int p_idx, std::string const& p_parameterName, Material& p_oMat)
  {
...
{
  auto LoadTextureType = [p_aiMaterial, this](aiTextureType p_type, std::string const& p_parameterName, Material& p_oMat )
  {
...
void ModelLoader::LoadAndComputeNormalMap(std::string const& p_directory, std::string const& p_fileName,
  Texture<unsigned char>& p_texture)
{
  std::string path = m_directory + "/" + p_fileName;
  std::replace( path.begin(), path.end(), '\\', '/');
